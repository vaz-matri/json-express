...
- **Custom Endpoints** - Extend your models immediately using the `endpoints: {}` block in schemas, or mount route-only files (search, webhooks, auth) with `defineRoutes` in `/models` — the filename becomes the mount prefix.
